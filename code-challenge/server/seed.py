...
with app.app_context():
        ...
        hero_power1 = hero_powers(strength = strengths[0],hero_id = hero3.id,power_id = power1.id)
        hero_power2 = hero_powers(strength = strengths[1],hero_id = hero7.id,power_id = power2.id)
        hero_power3 = hero_powers(strength = strengths[2],hero_id = hero2.id,power_id = power3.id)
        hero_power4 = hero_powers(strength = strengths[2],hero_id = hero4.id,power_id = power4.id)
        hero_power5 = hero_powers(strength = strengths[1],hero_id = hero9.id,power_id = power3.id)
        hero_power6 = hero_powers(strength = strengths[0],hero_id = hero5.id,power_id = power2.id)

        hp_instances = [hero_power1,hero_power2,hero_power3,hero_power4,hero_power4,hero_power5,hero_power6]
        db.session.add_all(hp_instances)
        db.session.commit()
